...
/**
 * A 6520 PIA (Parallel Interface Adapter) simulator Component.
 * <p>
 * This class wraps the {@link Sim6520} class and maps its in- and outputs to the {@link Node} interface.
 * <p>
 * Note: All inputs and outputs have the logic levels as a real 6520.
 * <p>
 */
public class MOS6520 extends Node implements Element {

    /**
     * The MOS6520 {@link ElementTypeDescription}
     */
    public static final ElementTypeDescription DESCRIPTION = new ElementTypeDescription(MOS6520.class, //
            input("CS1"), //
            input("~CS2"), //
            input("CS0"), //
            input("RS", "Register Adress [0..1]"), //
            input("R/~W"), //
            input("~RESET"), //
            input("PHI2").setClock(), //
            input("CA1"), //
    ...
    // The outputs
    private final ObservableValue irqaOut;
    private final ObservableValue irqbOut;
    ...
    private final Sim6520 sim6520;
    private final Sim6520Input sim_input;
    private Sim6520Output sim_output;
     ...
     */
    public MOS6520(ElementAttributes attr) {
        super(true);
        this.attr = attr;
        label = attr.getLabel();

        irqaOut = new ObservableValue("~IRQA", 1).setPinDescription(DESCRIPTION);
        ...
        irqbOut = new ObservableValue("~IRQB", 1).setPinDescription(DESCRIPTION);

        dataOut = new ObservableValue("D", 8).setToHighZ().setPinDescription(DESCRIPTION).setBidirectional();

        sim6520 = new Sim6520();
        sim_input = new Sim6520Input();
        sim_output = new Sim6520Output();
    }

    @Override
    public ObservableValues getOutputs() {
        return new ObservableValues(dataOut, irqaOut, ca2Out, paOut, padirOut, pbOut, pbdirOut, cb2Out, irqbOut);
    }
    ...
    public void setInputs(ObservableValues inputs) throws NodeException {
        ...
        cs0In = inputs.get(2).checkBits(1, this).addObserverToValue(this);
        rsIn = inputs.get(3).checkBits(2, this).addObserverToValue(this);
        rwIn = inputs.get(4).checkBits(1, this).addObserverToValue(this);
        resetIn = inputs.get(5).checkBits(1, this).addObserverToValue(this);
        clockIn = inputs.get(6).checkBits(1, this).addObserverToValue(this);
        ca1In = inputs.get(7).checkBits(1, this).addObserverToValue(this);
        cb1In = inputs.get(8).checkBits(1, this).addObserverToValue(this);
        paIn = inputs.get(9).checkBits(8, this).addObserverToValue(this);
        pbIn = inputs.get(10).checkBits(8, this).addObserverToValue(this);

        dataIn = inputs.get(11).checkBits(8, this).addObserverToValue(this);
        ca2In = inputs.get(12).checkBits(1, this).addObserverToValue(this);
        cb2In = inputs.get(13).checkBits(1, this).addObserverToValue(this);
    }
    ...
    public void readInputs() throws NodeException {
        ...
        try {
            ...
            sim_output = sim6520.tick(sim_input);

        } catch (Exception e) {
            throw new NodeException("Error in 6520 simulation", e);
        }
    }

    @Override
    public void writeOutputs() {
        irqaOut.setBool(sim_output.irqa);
        irqbOut.setBool(sim_output.irqb);
        ...
        if (sim_output.ca2_dir) {
            ca2Out.setBool(sim_output.ca2);
        } else {
            ca2Out.setToHighZ();
        }
        if (sim_output.cb2_dir) {
            cb2Out.setBool(sim_output.cb2);
        } else {
            cb2Out.setToHighZ();
        }

        if (sim_input.rw && sim_input.cs1 && !sim_input.cs2 && sim_input.cs0) {
